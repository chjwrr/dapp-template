...
import LoadingConfirmJson from '@/assets/lottlie/loading_confirm.json'
import LoadingErrorJson from '@/assets/lottlie/loading_error.json'
import LoadingPenddingJson from '@/assets/lottlie/loading_pending.json'
import LoadingSuccessJson from '@/assets/lottlie/loading_success.json'
...
function TransactionLoading() {
  ...
  return(
    props && <div className={`${styles.loadingView} animate__animated ${loadingAnimated} animate__faster`}>
      <img className={styles.close} onClick={onClose} src={'/svg/close.svg'}/>
      ...
      <div className={styles.msgView}>
        ...
        {props.hash && <span className={styles.msgViewon}
          onClick={()=>{
            window.open(getScanLink(props.chainID || 1,props.hash,"transaction"))
          }}>
